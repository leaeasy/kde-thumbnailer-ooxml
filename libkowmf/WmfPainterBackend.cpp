...
bool WmfPainterBackend::begin(const QRect &boundingBox)
{
    ...
    if (boundingBox.width() == 0 || boundingBox.height() == 0
        || mOutputSize.width() <= 0 || mOutputSize.height() <= 0) {
        return false;
    }

    const qreal scale = qMin(mOutputSize.width() / qAbs(boundingBox.width()),
                             mOutputSize.height() / qAbs(boundingBox.height()));
...
void WmfPainterBackend::setMatrix(WmfDeviceContext &context, const QTransform &wm, bool combine)
{
    Q_UNUSED(context);
#if DEBUG_WMFPAINT
