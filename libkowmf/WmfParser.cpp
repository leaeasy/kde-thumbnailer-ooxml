            ...
            case (META_TEXTOUT & 0xff):
                {
                    ...
                    m_backend->drawText(mDeviceContext, x, y, QString::fromLocal8Bit(text));
                }
                break;
            ...
            case (META_CREATEFONTINDIRECT & 0xff):
                {
                    ...
                    if (addHandle(handle)) {
                        ...
                        const int qtWeight = weight == 0 ? QFont::Normal : qBound(1, int(weight), 1000);
                        handle->font.setWeight(static_cast<QFont::Weight>(qtWeight));
                        handle->font.setItalic((property & 0x01));
                        handle->font.setUnderline((property & 0x100));
                        // TODO: Strikethrough

                        // font name
                        const int maxChar = size > 12 ? int((size - 12) * 2) : 0;
                        QByteArray nameFont(maxChar, Qt::Uninitialized);
                        if (maxChar > 0) {
                            stream.readRawData(nameFont.data(), maxChar);
                        }
                        const qsizetype terminator = nameFont.indexOf('\0');
                        if (terminator >= 0) {
                            nameFont.truncate(terminator);
                        }
                        handle->font.setFamily(QString::fromLocal8Bit(nameFont));
                    }
                }
                break;
...
bool WmfParser::addHandle(KoWmfHandle* handle)
{
    ...
    for (idx = 0; idx < mNbrObject ; idx++) {
        if (mObjHandleTab[ idx ] == nullptr)  break;
    }
