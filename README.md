...
KDE OOXML Thumbnailer 是一个面向 Qt 6 和 KDE Frameworks 6 的 KIO 缩略图插件，用于在 Dolphin 等 KDE 应用中显示 Microsoft Office Open XML 文档预览。

## 功能

- 支持 DOCX、XLSX 和 PPTX 文档。
- 支持标准 OOXML MIME 类型以及 WPS Office 注册的对应 MIME 类型。
...
- 为轻量级回退预览添加 DOCX、XLSX 或 PPTX 格式标识。内嵌缩略图保持原样。
- 对 XML 大小、文本数量、样式数量和输出像素数设置限制，以降低异常文档造成资源消耗的风险。

轻量级回退渲染器不是完整的 Office 排版引擎。复杂分页、图表、公式、嵌入对象、宏、动画以及部分主题字体或版式可能不会显示，或与 Office/WPS 中的效果存在差异。
...
```bash
sudo pacman -S base-devel cmake extra-cmake-modules ninja qt6-base karchive kcoreaddons kio
```
...
2. 打开“配置 Dolphin”。
3. 进入“常规”中的“预览”页面。
4. 启用“Microsoft Office Open XML 文档”。
5. 确保当前目录视图已启用文件预览。
...
回退渲染直接读取 OOXML ZIP 包中的有限 XML 内容，不启动外部程序，不执行宏，也不访问网络。文档仍应被视为不可信输入；如果发现解析异常、崩溃或资源消耗问题，请附带可公开的最小复现文件提交问题。
